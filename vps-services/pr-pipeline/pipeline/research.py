...
async def run_research(
    ...
    if dry_run:
        # --- DRY RUN: insert mock outlets instead of calling APIs ---
        logger.info(f"DRY RUN: Inserting 5 mock outlets for run {run_id}")
        mock_types = [
            ("DryRun-TV-1", "tv"),
            ("DryRun-TV-2", "tv"),
            ("DryRun-Magazine-1", "magazine"),
            ("DryRun-Podcast-1", "podcast"),
            ("DryRun-Blog-1", "blog"),
        ]
        for mock_name, mock_type in mock_types:
            outlet_data = {
                "name": mock_name,
                "outlet_type": mock_type,
                "url": f"https://example.com/{mock_name.lower()}",
                "description": f"Dry run mock outlet: {mock_name}",
                "audience_size": 10000,
                "topics": ["dry_run", "test"],
                "relevance_score": 70,
        ...
        # Still process seed outlets in dry_run mode
        ...
        return {"outlets_discovered": len(discovered_outlets), "total_cost": 0.0}
    ...
    # --- Exa searches (outlet discovery) ---
    exa_results = []
    exa_query_count = 0
    if exa and exa.available:
        industry = client.get("industry", "")
        country = territory.get("name", "")
        exa_queries = [
            f"{industry} media outlets and journalists in {country}",
            f"{industry} podcasts and shows accepting pitches {country}",
            f"PR contact {industry} publications {country}",
        ]
        for eq in exa_queries:
            try:
                results = await exa.search(eq, num_results=10)
                exa_results.extend(results)
                exa_query_count += 1
                await log_cost(
                    supabase, run_id, None, "exa", "search",
                    cost_usd=0.01, credits_used=1,
                    metadata={"query": eq, "results_count": len(results)},
                )
                total_cost += 0.01
            except Exception as e:
                logger.error(f"Exa search failed for '{eq}': {e}")
...
{json.dumps(all_tavily_results, indent=2)}"""
    ...
    # --- Parse Exa results with Claude ---
    if exa_results:
        try:
            exa_user_prompt = f"""Client: {client.get('name', '')} ({client.get('industry', '')})
Territory: {territory.get('name', '')} ({country_code})
Target markets: {json.dumps(client.get('target_markets', []))}

Search results to parse (from Exa neural search):
{json.dumps(exa_results, indent=2)}"""

            parsed_exa_outlets, exa_response = await claude.complete_json(
                system_prompt=RESEARCH_SYSTEM_PROMPT,
                user_prompt=exa_user_prompt,
            )
            total_cost += exa_response.cost_usd
            await log_cost(
                supabase, run_id, None, "anthropic", "research_parse_exa",
                cost_usd=exa_response.cost_usd,
                metadata={
                    "prompt_tokens": exa_response.prompt_tokens,
                    "completion_tokens": exa_response.completion_tokens,
                    "outlets_parsed": len(parsed_exa_outlets) if isinstance(parsed_exa_outlets, list) else 0,
                },
            )

            if isinstance(parsed_exa_outlets, list):
                for outlet_data in parsed_exa_outlets:
                    outlet_record = await _process_outlet(
                        outlet_data=outlet_data,
                        source_url=outlet_data.get("url", ""),
                        run_id=run_id,
            ...
            logger.error(f"Claude parsing of Exa results failed: {e}")
