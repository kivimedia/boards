...
export async function POST(
  ...
  if (phase === 'plan_review') {
    ...
    if (decision === 'approve') {
      ...
      // Create VPS job to resume pipeline from publishing phase (index 6)
      const { data: job } = await supabase
        .from('vps_jobs')
        .insert({
          job_type: 'seo',
          ...
          payload: {
            team_config_id: run.team_config_id,
            pipeline_run_id: id,
            resume_from_phase: 6, // publishing (after gate1 at index 5)
          },
        })
        .select('id')
        .single();
    ...
    if (decision === 'approve') {
      ...
      // Create VPS job to re-run visual_qa (index 7)
      const { data: job } = await supabase
        .from('vps_jobs')
        .insert({
          job_type: 'seo',
