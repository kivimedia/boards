...
function PasswordInput(props: React.InputHTMLAttributes<HTMLInputElement> & { label?: string }) {
  const [show, setShow] = useState(false);
  const { label, ...inputProps } = props;
  return (
    <div className="relative">
      <Input {...inputProps} label={label} type={show ? 'text' : 'password'} />
      <button
        type="button"
        onClick={() => setShow((v) => !v)}
        className="absolute right-3 top-[34px] text-navy/30 dark:text-white/30 hover:text-navy/60 dark:hover:text-white/60 transition-colors"
        tabIndex={-1}
      >
        {show ? (
