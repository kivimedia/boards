  ...
  const fetchCalendars = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Auto-generate if no calendars exist and silos are configured
        if (cals.length === 0 && autoGeneratedFor.current !== selectedConfigId) {
          const cfg = configs.find(c => c.id === selectedConfigId);
          const hasSilos = (cfg?.config?.content_targets || []).length > 0;
          if (hasSilos) {
            autoGeneratedFor.current = selectedConfigId;
            setLoading(false);
