...
const WEB_RESEARCH_TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      'Navigate to a URL using maximum stealth (Camoufox anti-bot browser). Bypasses Cloudflare, TLS fingerprinting, canvas fingerprinting. Use this when navigate_and_extract gets blocked (403, Cloudflare challenge, empty content). Slower but much harder to detect.',
    input_schema: {
      type: 'object' as const,
      properties: {
        url: { type: 'string', description: 'The URL to navigate to stealthily' },
      },
      required: ['url'],
    },
  },
  {
    ...
    description:
      'Extract specific elements from a page using CSS selectors with stealth browser. Use when scrape_elements fails due to anti-bot. Supports adaptive selectors that survive site redesigns.',
    input_schema: {
      type: 'object' as const,
      properties: {
        url: { type: 'string', description: 'The URL to scrape stealthily' },
        selectors: {
          type: 'array',
          items: { type: 'string' },
          description: 'CSS selectors to extract (e.g., ["h2.title", ".price", "a.product-link"])',
        },
      },
      required: ['url', 'selectors'],
    },
  ...
  try {
    switch (toolName) {
      ...
      case 'stealth_navigate':
        return await toolStealthNavigate(params, toolInput);
      case 'stealth_extract':
        return await toolStealthExtract(params, toolInput);
      default:
...
// ============================================================================
// SCRAPLING STEALTH TOOLS
// ============================================================================

async function toolStealthNavigate(
  params: WebResearchParams,
  input: Record<string, unknown>
): Promise<ToolResult> {
  const url = input.url as string;
  ...
  if (!available) {
    return { success: false, message: 'Scrapling stealth service not running. Use navigate_and_extract instead.' };
  }

  const result = await scraplingStealthy({ url: check.url, timeout: 45 });
  if (!result.success) {
    return { success: false, message: `Stealth fetch failed: ${result.error}` };
  }

  return {
    success: true,
    message: `[STEALTH] Title: ${result.title || '(none)'}\nURL: ${check.url}\nContent (${result.content_length || 0} chars):\n${(result.content || '').slice(0, 30_000)}`,
    data: { url: check.url, title: result.title, fetcher: result.fetcher_used },
  };
}

async function toolStealthExtract(
  params: WebResearchParams,
  input: Record<string, unknown>
): Promise<ToolResult> {
  const url = input.url as string;
  ...
  const available = await isScraplingAvailable();
  if (!available) {
    return { success: false, message: 'Scrapling stealth service not running. Use scrape_elements instead.' };
  }

  const result = await scraplingExtract({ url, selectors, fetcher: 'stealth' });
  if (!result.success) {
    return { success: false, message: `Stealth extract failed: ${result.error}` };
  }

  const parts: string[] = ['[STEALTH EXTRACT]'];
  for (const [sel, items] of Object.entries(result.results)) {
    parts.push(`## ${sel}`);
    for (const item of items.slice(0, 50)) {
      parts.push(`- ${item || '(empty)'}`);
    }
  }
