...
// ---------------------------------------------------------------------------
// CONVERSATIONAL AGENT (multi-turn with message history)
// Added post-pivot for agent chat sessions
// ---------------------------------------------------------------------------
...
export async function executeAgentConversation(
  ...
  try {
    ...
    return {
      fullOutput,
      updatedMessageHistory: updatedHistory,
      inputTokens: totalInputTokens,
      outputTokens: totalOutputTokens,
      costUsd,
      toolCallCount,
    ...
    callbacks.onError(err.message ?? 'Conversation error');
    throw err;
  }
}
