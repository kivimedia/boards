...
DIVI 5 COLUMN BUG: Divi 5 renders columns with class et_flex_column_24_24 which forces width:100%, breaking multi-column layouts. For multi-column rows, ALWAYS use the correct flexType: "8_24" for 3 columns, "6_24" for 4 columns, "12_24" for 2 columns. NEVER use "24_24" for columns that should sit side-by-side.`,
...
DIVI 5 FIX PATTERNS: Divi 5 forces et_flex_column_24_24 (width:100%) on every column, breaking multi-column layouts. Fix with a <!-- wp:html --><style>...</style><!-- /wp:html --> block containing: (1) Row-level CSS grid: .et_pb_row_N { display:grid !important; grid-template-columns:repeat(COLS,1fr) !important; } (2) Column width reset: .et_pb_row_N > .et_pb_column { width:auto !important; flex:none !important; } (3) For image-overlay cards: absolute-position the image, add ::after gradient overlay, z-index text container. WordPress strips <script> tags - CSS only.`,
