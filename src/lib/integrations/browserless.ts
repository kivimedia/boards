...
export class BrowserlessClient {
  ...
  async getContent(url: string): Promise<BrowserlessContentResult> {
    ...
    try {
      ...
      // Detect Cloudflare/anti-bot blocks and fall back to scrapling
      const isBlocked = response.status === 403 || response.status === 503
        || html.includes('cf-challenge') || html.includes('Just a moment...')
        || html.includes('Checking your browser');
      if (isBlocked) {
        return this.scraplingFallbackGetContent(check.url);
      }
  ...
  private async scraplingFallbackGetContent(url: string): Promise<BrowserlessContentResult> {
    ...
    if (!available) {
      throw new Error('Browserless returned anti-bot block and Scrapling service is not running. Start it with: cd scripts/scrapling-service && uvicorn server:app --port 8099');
    }

    const result = await scraplingTieredFetch(url);
    if (!result.success) {
      throw new Error(`Scrapling fallback also failed: ${result.error}`);
    }
  ...
  /**
   * Stealth fetch using Scrapling's Camoufox engine directly.
   * Bypasses Cloudflare Turnstile, TLS fingerprinting, canvas fingerprinting.
   * Use this when you know the target has aggressive anti-bot.
   */
  async stealthFetch(url: string): Promise<BrowserlessContentResult> {
    const check = sanitizeUrl(url);
    if (!check.valid) throw new Error(`URL blocked: ${check.reason}`);

    const available = await isScraplingAvailable();
    if (!available) {
      throw new Error('Scrapling service is not running. Start it with: cd scripts/scrapling-service && uvicorn server:app --port 8099');
    }

    const result = await scraplingStealthy({ url: check.url });
    if (!result.success) {
      throw new Error(`Stealth fetch failed: ${result.error}`);
    }
  ...
  /**
   * Extract specific CSS selectors using Scrapling's adaptive engine.
   * Uses element fingerprinting that survives site redesigns.
   */
  async stealthScrape(url: string, selectors: string[]): Promise<BrowserlessScrapeResult> {
    const check = sanitizeUrl(url);
    if (!check.valid) throw new Error(`URL blocked: ${check.reason}`);

    const available = await isScraplingAvailable();
    if (!available) {
      throw new Error('Scrapling service is not running');
    }

    const result = await scraplingExtract({ url: check.url, selectors, fetcher: 'stealth' });
    if (!result.success) {
      throw new Error(`Stealth scrape failed: ${result.error}`);
    }

    const data = selectors.map((selector) => ({
      selector,
      results: (result.results[selector] || []).map((text) => ({ text })),
    }));

    return { data, url: check.url };
  }
