...
export const SKILL_SEEDS: SkillSeed[] = [
  ...
  // =========================================================================
  // HISTORIAN TEAM (5-agent archival pipeline)
  // =========================================================================
  {
    slug: 'historian-orchestrator',
    name: 'Historian Orchestrator',
    ...
    strengths: [
      'Coordinates full archival pipeline across 4 specialist agents',
      'Owns final archive admission decisions (approved/rejected/review)',
      'Manages historian_configs for multi-source future expansion',
      'Batch-aware - processes images in manageable chunks',
      'Produces pipeline status reports with per-stage metrics',
    ],
    weaknesses: [
      'Depends on all 4 sub-agents being available',
      'Pipeline state not persisted between runs yet',
    ],
    improvement_suggestions: [
      'Add pipeline state persistence for crash recovery',
      'Implement priority queue for re-processing flagged images',
    ],
    supported_tools: [],
    required_context: ['historian_config_id', 'slack_channel'],
    output_format: 'Pipeline run summary with counts per stage and final archive stats',
    estimated_tokens: 4000,
    depends_on: [],
    feeds_into: ['historian-collector', 'historian-analyzer', 'historian-sidekick', 'historian-portal'],
    requires_mcp_tools: [],
    fallback_behavior: 'If sub-agents unavailable, run in single-agent mode with reduced capabilities.',
    reference_docs: [],
    icon: '🎯',
    color: '#D97706',
    sort_order: 30,
  ...
  {
    slug: 'historian-collector',
    name: 'Historian Collector',
    description: 'Pulls images from a Slack channel using the shared slack-image-fetcher agent. Extracts metadata (uploader, date, filename, thread context) and stores raw entries in historian_images. Supports 12-month lookback with pagination.',
    category: 'strategy',
    pack: 'custom',
    is_team_member: true,
    quality_tier: 'solid',
    ...
    strengths: [
      'Uses existing Slack OAuth token - no separate auth needed',
      'Handles Slack API pagination for large channels',
      'Deduplicates via unique constraint on (config_id, image_id)',
      'Extracts rich metadata: uploader, thread context, permalink',
      'Configurable lookback window from historian_configs',
    ],
    weaknesses: [
      'Requires valid Slack OAuth token with channels:history and files:read scopes',
      'Rate-limited by Slack API (tier 3)',
    ],
    improvement_suggestions: [
      'Add incremental collection (only new images since last run)',
      'Support Google Drive and Dropbox as alternative sources',
    ],
    supported_tools: [],
    required_context: ['historian_config_id', 'slack_channel', 'lookback_months'],
    output_format: 'Array of collected image records with Slack metadata',
    estimated_tokens: 3000,
    depends_on: ['historian-orchestrator', 'slack-image-fetcher'],
    feeds_into: ['historian-analyzer'],
    requires_mcp_tools: [],
    fallback_behavior: 'If Slack token invalid, report auth error to Orchestrator.',
    reference_docs: [],
    icon: '📥',
    color: '#2563EB',
    sort_order: 31,
  ...
  {
    slug: 'historian-analyzer',
    name: 'Historian Analyzer',
    description: 'Analyzes collected images using vision-capable models. Enriches with Humanity shift data (client, location, event) via the shared humanity-matcher agent. Generates titles, descriptions, tags, categories, and 5-star quality scores with dimensional breakdowns. Produces keep/reject/review recommendations for the Orchestrator.',
    category: 'strategy',
    pack: 'custom',
    is_team_member: true,
    quality_tier: 'solid',
    ...
    strengths: [
      'Structured 5-star quality scoring with dimensional breakdowns',
      'Generates rich metadata: title, description, tags, category',
      'Clear keep/reject/review recommendation logic',
      'Archival notes capture historical context and significance',
      'Enriches images with Humanity shift data via shared humanity-matcher agent (client, location, crew, product)',
    ],
    weaknesses: [
      'Requires vision-capable model for image analysis',
      'Quality scoring subjectivity in edge cases',
    ],
    improvement_suggestions: [
      'Add automatic re-scoring when Sidekick produces a cleaned version',
      'Implement confidence scores for category assignments',
    ],
    supported_tools: [],
    required_context: ['image_url', 'image_metadata', 'client_context'],
    output_format: 'JSON with title, description, tags, category, quality_score, scoring_dimensions, keep_decision, rationale',
    estimated_tokens: 4000,
    depends_on: ['historian-collector', 'humanity-matcher'],
    feeds_into: ['historian-sidekick'],
    requires_mcp_tools: [],
    fallback_behavior: 'If vision model unavailable, generate metadata from filename and Slack context only. If Humanity API unavailable, skip event attribution.',
    reference_docs: [],
    icon: '🔍',
    color: '#059669',
    sort_order: 32,
  ...
  {
    slug: 'historian-sidekick',
    name: 'Historian Sidekick',
    ...
    strengths: [
      ...
      'Structured output with risk levels and recommendations',
      'Escalation rules for sensitive or ambiguous images',
    ],
    weaknesses: [
      'Depends on Nano Banana 2 or equivalent image editing capability',
      'Cannot make archive admission decisions - Orchestrator is final judge',
    ...
    supported_tools: ['post_comment', 'update_custom_field'],
    required_context: ['image_file', 'image_id', 'analyzer_assessment'],
    output_format: 'JSON with image_id, decision, edit_summary, risk_level, original_path, clean_path',
    estimated_tokens: 3000,
    depends_on: ['historian-analyzer'],
    feeds_into: ['historian-portal'],
    requires_mcp_tools: [],
  ...
  {
    slug: 'historian-portal',
    name: 'Historian Portal',
    ...
    strengths: [
      'Client-friendly archive browsing by category, date, and client',
      'Conversational interface for querying archive history',
      'Generates shareable gallery views for client presentations',
      'Filters by quality score, category, and approval status',
    ],
    weaknesses: [
      'Requires approved images to exist in historian_images first',
      'Gallery generation depends on frontend components not yet built',
    ],
    improvement_suggestions: [
      'Add export to PDF/slideshow for client presentations',
      'Implement search-by-similar-image using embeddings',
    ],
    supported_tools: [],
    required_context: ['historian_config_id', 'client_id'],
    output_format: 'Rendered archive views with organized image galleries',
    estimated_tokens: 3000,
    depends_on: ['historian-sidekick'],
    feeds_into: [],
    requires_mcp_tools: [],
    fallback_behavior: 'If no approved images, show empty state with collection status.',
    reference_docs: [],
    icon: '🖼️',
    color: '#EC4899',
    sort_order: 34,
  ...
  // =========================================================================
  // SHARED AGENTS (used by multiple teams - Historian + SEO)
  // =========================================================================
  {
    slug: 'slack-image-fetcher',
    name: 'Slack Image Fetcher',
    description: 'Generic Slack channel image collector. Fetches images from any configured Slack channel with metadata extraction (uploader, timestamp, thread context, permalink). Supports pagination, date filtering, and deduplication. Used as a shared utility by both Historian and SEO teams.',
    category: 'strategy' as const,
    pack: 'shared' as const,
    is_team_member: false,
    quality_tier: 'solid' as const,
    quality_score: 77,
    quality_notes: 'Extracted and unified from slack-seo.ts and historian-collector patterns. Single source of truth for Slack image ingestion.',
    strengths: [
      'Unified Slack image fetching - no code duplication between teams',
      'Dual strategy: conversations.history + files.list for completeness',
      'Rich metadata extraction: uploader, thread, permalink, MIME type, file size',
      'Supports both SEO and Historian token sources via configType discriminator',
      'Date windowing with oldest/newest filters for incremental collection',
    ],
    weaknesses: [
      'Requires valid Slack OAuth token with channels:history and files:read scopes',
      'Rate-limited by Slack API (tier 3)',
    ...
    improvement_suggestions: [
      'Add webhook listener for real-time image notifications',
      'Support additional sources (Google Drive, Dropbox) via adapter pattern',
    ],
    supported_tools: [],
    required_context: ['config_id', 'config_type', 'channel_id'],
    output_format: 'Array of SlackImageResult objects with URLs and metadata',
    estimated_tokens: 2000,
    depends_on: [],
    feeds_into: ['historian-analyzer', 'seo-featured-image'],
    requires_mcp_tools: [],
    fallback_behavior: 'If Slack token invalid, report auth error with refresh instructions.',
    reference_docs: [],
    icon: '📷',
    color: '#4A154B',
    sort_order: 38,
  ...
  {
    slug: 'humanity-matcher',
    name: 'Humanity Shift Matcher',
    description: 'Takes a timestamp (from Slack image or manual input) and finds the nearest matching shift in Humanity (stagesplus.humanity.com). Enriches with client name, location, crew, employees, product type, and event details. Returns structured match data for archival attribution and SEO image context.',
    category: 'strategy' as const,
    pack: 'shared' as const,
    is_team_member: false,
    quality_tier: 'solid' as const,
    quality_score: 75,
    quality_notes: 'Time-proximity matching against Humanity shifts API. Parses shift titles for client/product info. Caches locations for performance.',
    strengths: [
      'Automatic shift matching by timestamp proximity with configurable window',
      'Parses shift titles to extract client name and product type',
      'Extracts address, contact, invoice from shift notes',
      'Three-tier confidence scoring (high/medium/low) based on time delta',
      'In-memory caching for locations (10-min TTL)',
      'Used by both Historian (archival attribution) and SEO (image context)',
    ],
    weaknesses: [
      'Depends on Humanity API availability and valid access token',
      'Matching by time proximity may be ambiguous for overlapping shifts',
    ],
    improvement_suggestions: [
      'Add location-based matching as secondary signal',
      'Support batch matching for multiple timestamps in one call',
      'Add shift type / product type inference from schedule metadata',
    ],
    supported_tools: [],
    required_context: ['humanity_config', 'timestamp'],
    output_format: 'HumanityMatchResult JSON with shift, client, location, confidence, rationale',
    estimated_tokens: 2000,
    depends_on: ['slack-image-fetcher'],
    feeds_into: ['historian-analyzer'],
    requires_mcp_tools: [],
    fallback_behavior: 'If Humanity API unavailable, return { matched: false } with error message. Pipeline continues without enrichment.',
    reference_docs: [],
    icon: '🔗',
    color: '#0891B2',
    sort_order: 39,
  ...
  // =========================================================================
  // GOOGLE ADS INTEGRATION (3 skills)
  // =========================================================================
  {
    slug: 'google-ads-manager',
    name: 'Google Ads Manager',
    description: 'Google Ads account management agent. Monitors campaigns, keywords, and search terms. Identifies wasted spend and paid-to-organic transition opportunities using the WASTE Score framework.',
    category: 'ads',
    pack: 'seo_team',
    quality_tier: 'genuinely_smart',
    quality_score: 83,
    quality_notes: 'Major rewrite with WASTE Score framework (Wasted spend / Ad relevance / Search intent / Transition readiness / Efficiency ratio), 4-step gradual transition protocol, budget reallocation formula, QS diagnosis checklist, 3 worked examples (e-commerce branded terms, SaaS content gap, local budget bleed), multi-tier fallback, explicit limitations, cost estimation.',
    strengths: [
      'WASTE Score framework quantifies keyword health with 5 weighted factors - actionable, not subjective',
      'Gradual transition protocol (30% bid reduction per week) prevents cold-turkey traffic dips',
      'Anti-patterns section prevents common mistakes (premature pause, broad negatives, QS gaming)',
      '3 diverse worked examples covering e-commerce, SaaS, and local business scenarios',
      'Budget reallocation formula with conservative 50% move and ROAS targeting',
      'Multi-tier fallback from live API to cache to manual export instructions',
    ],
    weaknesses: [
      'No historical trend analysis yet - only point-in-time snapshots',
      'Cannot cross-reference with Search Console data automatically (requires pipeline integration)',
      'WASTE Score weights need calibration from real usage data across multiple accounts',
    ],
    improvement_suggestions: [
      'Add time-series budget tracking with 30/60/90-day trend visualization',
      'Integrate Search Console position data for automated transition scoring',
      'Build WASTE Score calibration dataset from first 10 client accounts',
    ],
    supported_tools: ['think', 'gads_list_campaigns', 'gads_keyword_performance', 'gads_search_terms_report', 'gads_budget_overview', 'gads_update_budget', 'gads_pause_keyword'],
    required_context: ['team_config_id', 'google_ads_credentials'],
    output_format: 'markdown',
    estimated_tokens: 3000,
    depends_on: [],
    feeds_into: ['seo-strategy-planner', 'seo-content-writer'],
    requires_mcp_tools: ['gads_list_campaigns', 'gads_keyword_performance', 'gads_search_terms_report', 'gads_budget_overview'],
    fallback_behavior: '5-tier fallback: live API -> cache <24h (stale warning) -> cache <7d (directional only) -> manual CSV export instructions -> credential setup guide.',
    reference_docs: [],
    is_team_member: true,
    icon: '💰',
    color: '#F59E0B',
  ...
  },
  {
    ...
    description: 'Competitive intelligence agent. Pulls and analyzes competitor ad creatives using the GAPS Analysis framework (Geography/Audience/Positioning/Seasonal). Produces competitive landscape reports for pitches and content differentiation.',
    category: 'ads',
    pack: 'seo_team',
    quality_tier: 'genuinely_smart',
    quality_score: 80,
    quality_notes: 'Major rewrite with GAPS Analysis framework (Geography gaps / Audience blind spots / Positioning voids / Seasonal timing), 1-5 scoring system, anti-patterns for each job, 3 worked examples (SaaS 5-competitor landscape, local HVAC pitch prep, brand bidding detection), explicit limitations on spend estimation, multi-tier fallback, cost estimation.',
    strengths: [
      'GAPS framework (Geography/Audience/Positioning/Seasonal) provides structured competitive analysis with 1-5 scoring',
      'Anti-patterns prevent common mistakes (alphabetical listing, describing without interpreting, ignoring small competitors)',
      'Content Differentiation Brief format directly feeds Content Writer with "everyone says / nobody says / opportunity" structure',
      '3 worked examples: SaaS market landscape, local service pitch, brand bidding response',
      'Explicit spend estimation guardrails (ranges only, never precise competitor CPCs)',
      'Multi-tier fallback from API to cache to manual Transparency Center instructions',
    ],
    weaknesses: [
      'Dependent on Google Ads Transparency Library availability and data freshness (24-48h lag)',
      'Cannot estimate actual competitor spend - only ranges inferred from ad frequency',
      'Video analysis requires Gemini API key that most teams will not have configured',
    ],
    improvement_suggestions: [
      'Add historical ad tracking for competitor strategy pivot detection over time',
      'Build competitor messaging database with diff alerts when messaging changes',
      'Add automated alerts when new competitor enters a tracked keyword space',
    ],
    supported_tools: ['think', 'gads_competitor_ads', 'gads_ad_details', 'gads_analyze_ad_image', 'gads_analyze_ad_video'],
    required_context: ['team_config_id', 'competitor_domains'],
    output_format: 'markdown',
    estimated_tokens: 3000,
    depends_on: [],
    feeds_into: ['seo-content-writer', 'seo-strategy-planner'],
    requires_mcp_tools: ['gads_competitor_ads', 'gads_ad_details'],
    fallback_behavior: '5-tier fallback: live API -> cache <48h -> cache <7d (directional only) -> manual Transparency Center instructions with URL -> credential setup guide.',
    reference_docs: [],
    is_team_member: true,
    icon: '🔍',
    color: '#6366F1',
  ...
  },
  {
    slug: 'security-chief',
    name: 'Security Chief',
    description: 'Security audit agent using THREAT Matrix (Tool/Historical/Rate/External/Anomaly/Timing). Reviews MCP outputs for prompt injection, data leakage, and suspicious patterns. Reasoning-only with zero external access by design.',
    category: 'meta',
    pack: 'seo_team',
    quality_tier: 'genuinely_smart',
    quality_score: 82,
    quality_notes: 'Major rewrite with THREAT Matrix framework (Tool source / Historical pattern / Rate & rhythm / External indicator / Anomaly burst / Timing correlation), 4-tier tool risk classification, rate thresholds with burst and slow-drip detection, known attack signatures table, decision trees for escalation and false positive assessment, 3 worked examples (false positive ad HTML, real injection via search term poisoning, burst pattern API compromise), 30-day calibration protocol.',
    strengths: [
      'THREAT Matrix provides systematic 6-factor analysis framework - no flag goes unanalyzed',
      'Tool Risk Tiers (High/Medium/Low/Internal) set correct expectations per data source',
      'Decision trees for escalation and false positive assessment remove subjective judgment',
      '3 worked examples covering false positive, real injection, and burst attack scenarios',
      'Rate thresholds with burst (5+/hour) and slow-drip (1-2/hour for 12h) pattern detection',
      'Zero external access by design - audit agent cannot be weaponized for data exfiltration',
      '30-day calibration protocol establishes baseline before making severity judgments',
    ],
    weaknesses: [
      'Cannot block in real-time - only post-hoc review (sanitizer handles real-time blocking)',
      'No machine learning - pure reasoning-based pattern matching',
      'Known attack signatures table requires manual updates as new patterns emerge',
    ],
    improvement_suggestions: [
      'Add anomaly scoring model trained on 30-day baseline data',
      'Enable real-time webhook alerting for CRITICAL findings to Slack/email',
      'Add security health score dashboard widget showing 7-day trend',
    ],
    supported_tools: ['think'],
    required_context: ['security_audit_log_data'],
    output_format: 'markdown',
    estimated_tokens: 2500,
    depends_on: [],
    feeds_into: [],
...
export const IMPROVEMENT_LOG: ImprovementEntry[] = [
  ...
  },
  {
    slug: 'google-ads-manager',
    change_type: 'major_rewrite',
    change_description: 'Added WASTE Score framework (Wasted spend / Ad relevance / Search intent / Transition readiness / Efficiency ratio) with scoring rules and calculation formula. 4-step gradual transition protocol (30% bid reduction per week with safety checks). Budget reallocation formula with conservative 50% move. QS diagnosis checklist with anti-patterns. 3 worked examples: e-commerce branded terms, SaaS content gap discovery, local business budget bleed. Multi-tier 5-level fallback protocol. Cost estimation per operation. Explicit limitations section. Changed category from seo to ads.',
    quality_score_before: 72,
    quality_score_after: 83,
    quality_tier_before: 'solid',
    quality_tier_after: 'genuinely_smart',
  },
  {
    slug: 'competitive-intel',
    change_type: 'major_rewrite',
    change_description: 'Added GAPS Analysis framework (Geography gaps / Audience blind spots / Positioning voids / Seasonal timing) with 1-5 scoring system. Anti-patterns for each key job (no alphabetical listing, interpret not describe, do not ignore small competitors). Content Differentiation Brief format with everyone/nobody/opportunity structure. 3 worked examples: SaaS 5-competitor landscape, local HVAC pitch prep, brand bidding detection and response. Spend estimation guardrails (ranges only). Multi-tier 5-level fallback. Cost estimation. Changed category from seo to ads.',
    quality_score_before: 70,
  ...
  },
  {
    slug: 'security-chief',
    change_type: 'major_rewrite',
    change_description: 'Added THREAT Matrix framework (Tool source / Historical pattern / Rate & rhythm / External indicator / Anomaly burst / Timing correlation). 4-tier tool risk classification (High/Medium/Low/Internal) with expected flag types. Rate thresholds with burst (5+/hour) and slow-drip (1-2/hour for 12h) detection. Known attack signatures table. Decision trees for escalation and false positive assessment. 3 worked examples: false positive from ad HTML, real injection via search term poisoning, burst pattern suggesting API compromise. 30-day calibration protocol for baseline establishment.',
    quality_score_before: 74,
    quality_score_after: 82,
    quality_tier_before: 'solid',
    quality_tier_after: 'genuinely_smart',
  },
